...
class PostPagesTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.post = Post.objects.bulk_create(articles)
    ...
    def test_first_page_contains_ten_records(self):
        """Первая страница содержит n записей."""
        address_list = {
        ...
        for address, list in address_list.items():
            with self.subTest(address=address):
                ...
                self.assertEqual(
                    len(response.context[list]),
                    NUMBER_OF_POSTS_PER_PAGE
                )

    def test_second_page_contains_three_records(self):
        """Вторая страница содержит N-n записей."""
        address_list = {
            reverse('posts:index'): 'page_obj',
            reverse(
                'posts:group_posts',
        ...
        for address, list in address_list.items():
            with self.subTest(list=list):
                ...
                self.assertEqual(
                    len(response.context[list]),
                    Post.objects.all().count()-NUMBER_OF_POSTS_PER_PAGE
                )

    def test_post_group2_not_in_posts_group(self):
        """Пост из группы 2 не попал в группу 1."""
        response = self.authorized_client.get(
        ...
        post_group_0 = first_object.group.title
        post_text_0 = first_object.text
        self.assertNotEqual(post_group_0, self.group2.title)
